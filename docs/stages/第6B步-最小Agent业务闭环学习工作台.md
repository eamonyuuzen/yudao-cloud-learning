...
1. 用上面的框架无关伪代码手动跑一遍。
2. 对照 Spring AI、OpenAI Agents SDK、smolagents 的最小样例。
3. 只比较它们把循环的哪些部分封装了。
...
1. 能画出 Agent 工具循环并解释每个返回位置。
2. 能读懂 Yudao 当前 Java 工具链。
3. 能设计并实现一个只读工具契约。
4. 能让 Python Agent 通过正常 HTTP + Authorization 复用 Java。
5. 能区分 400、401、403、空结果、超时和模型错误。
6. 能检查轨迹和最终数据库事实。
7. 能说明 Spring AI、Python SDK、MCP、LangGraph 哪些是可替换层。
